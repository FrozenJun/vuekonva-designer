import { TheVisualDesignApi } from './../../../../../configuration/visual-design/src/TheVisualDesign.api';
...
export class BsHeaderConfig extends UIComponentFactory {
  ...
  async save(isAutoSave?: boolean) {
    ...
    const res = this.isTemplate
      ? await TheVisualDesignApi.templateSave(saveContent)
  ...
  publish() {
    ...
    this.$confirm('是否确认发布?', '提示', {
      ...
      .then(async () => {
        ...
        const res = this.isTemplate
          ? await TheVisualDesignApi.templatePublish({ id })
